...
for cnt in range(1, 13):
    ...
    for i in range(len(model_num)):
        ...
        for j in range(1):
            ...
            command = "python3 inference_intHigh.py --checkpoint_path ./data/small_model_mon%d/model_%d_%d/model-%d.weights.h5 --summary_dir ./summary_%d_%d --visual_dir ./vis_%d_%d --data_dir ./data/small_esrd_intHigh/small_esrd_%d_%d.npy --test_idx ./data/small_esrd_intHigh/case%d_bird_test_idx_%d.npy --r_dim 332 --r_max_dim 332 --mon %d --case %d | tee ./data/small_map_data_%d/%s" % (cnt, i, j, model_num[cur], i, j, i, j, i, j, i, j, j, i, cnt, j_name)
